...
RUN apt-get update \
    && apt-get install -y libzip-dev vim nano libicu-dev libonig-dev python3 pip python3-dev default-libmysqlclient-dev build-essential pkg-config systemctl default-mysql-client libfreetype6-dev libjpeg62-turbo-dev libpng-dev \
...
# RUN crontab < /tmp/crontab && rm -f /tmp/crontab && systemctl enable cron.service
